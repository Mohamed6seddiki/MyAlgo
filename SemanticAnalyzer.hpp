    ...
    void generateExpression(const std::shared_ptr<ExpressionNode>& expr, bool needParens = false) {
        if (auto num = std::dynamic_pointer_cast<NumberNode>(expr)) {
            ...
            if (op == "DIV") {
                ...
                // For simple arithmetic, don't always need parentheses
                bool simpleOp = (op == "+" || op == "-" || op == "*" || op == "/" || 
                               op == "<" || op == ">" || op == "<=" || op == ">=" || op == "==");
                if (needParens && !simpleOp) outputFile << "(";
                generateExpression(binOp->left, op == "+" || op == "-");
                outputFile << " " << op << " ";
                generateExpression(binOp->right, op == "+" || op == "-" || op == "*" || op == "/");
                if (needParens && !simpleOp) outputFile << ")";
            }
        }
